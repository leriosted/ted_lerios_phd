...
for jj = 1:length(breaths)
    ...
    RMSE(jj,1) = rmse(Peff,Pes,'all');
%     RMSE(jj,1) = sqrt(mean((Peff-Pes).^2))./(Peff);
...
ylabel('min P_{eff,3} [cmH_2O]') % CHANGE THIS
xlim([-100 0])
ylim([-60 0])
...
TextH = text(0,1,str, ...
  ...
  'VerticalAlignment', 'top','BackgroundColor','#FFFFFF');
fontsize(FigH,25,"pixels")
filename2 = append('Patient_cc_',num2str(kk),'A3'); % CHANGE THIS
print('-r400','-dpng',filename2);
pp_RMSE(kk,1) = median(RMSE);
pp_E(kk,1) = medE;
pp_R(kk,1) = medR;
% pp_r(kk,1) = tmp(1,2);
