function [P_model_lin,P_model_R2,P_model_Peff,E,R1_insp,R1_exp,R2,Phi,Peff,Peff_coeff,Psi,...
    ...
    = model_function(t_P,P,t_Q,Q,V,P0,exp_0,exp_max)
...
[t_spline,y_spline_Peff] = b_spline_basis_functions(M,2,length((1:exp_0-1))/100);
Psi = zeros(length(t_P),M); % \Psi = Splines_Overall_Peff
Psi(1:exp_0-1,:) = y_spline_Peff;
lb = [0;0];
ub = [inf;inf];

ind = [1:exp-1,exp_max-5:exp_max+5];
size(V(ind))
size(Psi(1:length(ind),:))
A_Peff = [(V(ind)-V(1)) Psi(1:length(ind),:)];
b_Peff = P(ind);
x_Peff = lsqlin(A_Peff,b_Peff,[],[],[],[],lb,ub);
% E = x_Peff(1); TESTING 
E = mean(-P(ind))./mean((V(ind)-V(1)));
Peff_coeff = x_Peff(2:end);
Peff = Psi(1:exp_0-1,:)*Peff_coeff;
P_model_Peff(1:exp_0-1) = E*V(1:exp_0-1) + Peff;
...
RMSE_Peff = sqrt(mean((P(1:exp_0-1) - P_model_Peff).^2));
