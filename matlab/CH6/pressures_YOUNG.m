...
if atWork == 1
    ...
    figPos1 = [2580, -190, 1000, 850]; % Home
...
% young
subplot(5,5,1)
plot(young.time,young.pressure(1,1:end),'Color',colorBlack,'Linewidth',pt,'Linestyle','-'); hold on
% plot(young.time(1:young.exp_point(1)-1),young.Peff_Array{1, 1},'Color',colorRed,'Linewidth',pt,'Linestyle','-.')
plot(young.time,young.Peff_Array{1, 1},'Color',colorRed,'Linewidth',pt,'Linestyle','-.')
xlabel('t (s)')
ylabel('P (cmH_2O)')
leg1 = legend('P_{alv}','P_{eff}','FontSize',12,...
    'Orientation','horizontal');
legend boxoff
leg1.Position(1:2) = [.75 .20];
xlim(tlim)
ylim(plim)
grid on
set(gca,'FontSize',FS);
...
% subplot(4,5,2)
% plot(young.time,young.pressure(2,1:end),'Color',colorBlack,'Linewidth',pt,'Linestyle','-'); hold on
% plot(young.time,young.Peff_Array{1, 2},'Color',colorRed,'Linewidth',pt,'Linestyle','-.')
% xlabel('t (s)')
% ylabel('P (cmH_2O)')
% xlim(tlim)
% ylim(plim)
...
for ii = 2:20
    subplot(5,5,ii)
    plot(young.time,young.pressure(ii,1:end),'Color',colorBlack,'Linewidth',pt,'Linestyle','-'); hold on
%     plot(young.time(1:young.exp_point(ii)-1),young.Peff_Array{1, ii},'Color',colorRed,'Linewidth',pt,'Linestyle','-.')
    plot(young.time,young.Peff_Array{1, ii},'Color',colorRed,'Linewidth',pt,'Linestyle','-.')
    xlabel('t (s)')
    ylabel('P (cmH_2O)')
    xlim(tlim)
    ylim(plim)
...
%        
if print_on ==1
    print('-r400','-dpng','pressuresYOUNG.png');
else 
end
